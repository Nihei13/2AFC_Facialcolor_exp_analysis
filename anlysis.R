...
if(!file.exists(paste(filesave_path, "fit_data.Rdata"))){
...
for (i in 1:length(df_sub$Subject)){
  ...
  sub_thre <- data.frame(sub = rep(sub_name[i], times=4), fit_sub$thresholds[1:3])

  sub_thre_all <- rbind(sub_thre_all,sub_thre)

  # plotting_PSE(fit_sub,sprintf('%s%s.png', filesave_path, sub_name[i]))
}
save(sub_thre_all, file = paste(filesave_path, "fit_data.Rdata"))
##
}else{
  load(paste(filesave_path, "fit_data.Rdata"))
}

df_conc <- aggregate(CorrectedResponse ~ FacialColor + Race + Anger_Morph_level + N, data = df_hoge, mean)

fit <- quickpsy(df_conc, Anger_Morph_level, CorrectedResponse, N, 
                grouping = .(FacialColor, Race))
fit
